...
const Player = () => {
    ...
    const markLectureAsCompleted = async (lectureId) => {
        try {
            const {data} = await axios.post(backendUrl + "/api/users/update-course-progress", {courseId, lectureId}, {withCredentials: true})
    ...
    const getCourseProgress = async () => {
        try {
            const {data} = await axios.post(backendUrl + "/api/users/get-course-progress", {courseId}, {withCredentials: true})
    ...
    const handleRate = async (rating) => {
        try {
            const {data} = await axios.post(backendUrl + "/api/users/add-rating", {courseId, rating}, {withCredentials: true})
                                            ...
                                            <li key={i} className='flex items-start gap-2 py-1'>
                                                ...
                                                <div className='flex items-center justify-between w-full text-gray-800 text-xs md:text-base'>
                                                    ...
                                                    <div className='flex gap-2'>
                                                        ...
                                                        })} className='text-[#1269e2] cursor-pointer'>Watch</p>}
                ...
                <div className='md:mt-10'>
                    {
                        ...
                        <div>
                            ...
                            <div className='flex justify-between items-center mt-1'>
                                ...
                                <button onClick={() => markLectureAsCompleted(playerData.lectureId)} className='text-[#1269e2]'>{progressData && progressData.lectureCompleted.includes(playerData.lectureId) ? 'Completed' : 'Mark complete' }</button>
                            </div>
                        </div>
                        :
